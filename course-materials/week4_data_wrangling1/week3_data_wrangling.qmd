...
#### More Exercises for summarise

1.  Calculate the average departure delay (`dep_delay`) for all flights.

2.  Find the maximum arrival delay (`arr_delay`) and the minimum departure delay (`dep_delay`).

3.  Determine the number of distinct destinations (`dest`) in the dataset.

4.  Calculate the total number of flights operated by each carrier.

5.  Find the average air time (`air_time`) for flights to each destination.

6.  Calculate the standard deviation of departure delays (`dep_delay`).

7.  Find the number of flights that departed each month.

8.  Calculate the average distance flown by each carrier.

9.  Find the maximum and minimum distances flown, and the average distance flown.

10. Determine the number of flights that departed from each origin (`origin`) and went to each destination (`dest`).
...
#### More exercises for combining multiple operations

1.  Find the average arrival delay for flights from each origin, but only for flights with an air time greater than 200 minutes.

2.  For each carrier, find the destination with the maximum average departure delay.

3.  Calculate the total number of flights and the average distance for each month and day combination, then find the top 5 combinations with the highest number of flights.

4.  Find the carrier with the highest average speed (distance/air_time) for flights to Los Angeles (LAX).

5.  Calculate the number of flights and the average departure delay for each origin-destination pair, but only for pairs with more than 100 flights.

6.  For each month, find the day with the highest number of cancelled flights (missing `dep_time`).

7.  Find the top 3 destinations with the highest average arrival delay for flights operated by United Airlines (UA).

8.  Calculate the average air time for flights operated by each carrier to each destination, but only for destinations with an average air time greater than 300 minutes.

9.  Find the origin with the highest average departure delay for flights in July.

10. For each destination, find the carrier with the smallest average air time.
